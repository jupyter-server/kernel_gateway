...
class SwaggerCellParser(LoggingConfigurable):
    ...
    operation_response_indicator = Unicode(
        default_value=r"{}\s*ResponseInfo\s+operationId:\s*(.*)"
    )
    notebook_cells = List()
        ...
        if "paths" in self.swagger:
            for endpoint in self.swagger["paths"].keys():
                for verb in self.swagger["paths"][endpoint].keys():
                    if "operationId" in self.swagger["paths"][endpoint][verb]:
                        operationId = self.swagger["paths"][endpoint][verb][
                            "operationId"
                        ]
            ...
            for operationId in operationIdsDeclared:
                if operationId not in operationIdsFound:
                    self.log.warning(
                        f"Operation {operationId} was declared but not referenced in a cell"
                    )
            for operationId in operationIdsFound:
                if operationId not in operationIdsDeclared:
                    self.log.warning(
                        f"Operation {operationId} was referenced in a cell but not declared"
                    )
        else:
    ...
    def endpoint_responses(self, source_cells, sort_func=first_path_param_index):
        ...
        """
        endpoints = self._endpoint_verb_source_mappings(
            source_cells, self.kernelspec_operation_response_indicator
        )
        sorted_keys = sorted(endpoints, key=sort_func, reverse=True)
        return [(key, endpoints[key]) for key in sorted_keys]
    ...
    def _endpoint_verb_source_mappings(self, source_cells, operationIdRegex):
        ...
        for endpoint in self.swagger["paths"].keys():
            for verb in self.swagger["paths"][endpoint].keys():
                if (
                    "operationId" in self.swagger["paths"][endpoint][verb]
                    and self.swagger["paths"][endpoint][verb]["operationId"]
                    in operationIds
                ):
                    ...
                    if "parameters" in self.swagger["paths"][endpoint][verb]:
                        ...
                        for parameter in self.swagger["paths"][endpoint][verb][
                            "parameters"
                        ]:
                            if "name" in parameter:
                                endpoint_with_param = "/:".join(
                                    [endpoint_with_param, parameter["name"]]
                                )
                        mappings.setdefault(endpoint_with_param, {}).setdefault(
                            verb, ""
                        )
