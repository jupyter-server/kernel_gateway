...
class KernelGatewayApp(JupyterApp):
    ...
    def _load_api_module(self, module_name):
        ...
        try:
            ...
            # a ValueError will be triggered in the caller
            pass
        return api_module
    ...
    def init_configurables(self):
        ...
        func = getattr(api_module, 'create_personality', None)
        if func is None:
            self.log.critical('ERROR: the personality factory function was not found in the API module {}'.format(self.api))
            self.exit(1)
        self.personality = func(self, self.kernel_manager.seed_kernelspec)

        self.personality.init_configurables(self.log, {
            'allow_notebook_download': self.allow_notebook_download,
            'prespawn_count': self.prespawn_count,
            'kernel_manager': self.kernel_manager,
