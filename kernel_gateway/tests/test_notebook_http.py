...
class TestDefaults:
    ...
    async def test_api_get_endpoint_with_query_param(self, jp_fetch):
        ...
        response = await jp_fetch(
            "hello", "person", params={"person": "governor"}, method="GET"
        )
        assert response.code == 200, "GET endpoint did not return 200."
    ...
    async def test_blocked_download_notebook_source(self, jp_fetch):
        ...
        assert (
            e.value.code == 404
        ), "/_api/source found when allow_notebook_download is false"
...
@pytest.mark.parametrize(
    "jp_argv", (["--NotebookHTTPPersonality.allow_notebook_download=True"],)
)
class TestSourceDownload:
    ...
    async def test_download_notebook_source(self, jp_fetch, jp_argv):
        ...
        assert (
            response.code == 200
        ), "/_api/source did not correctly return the downloaded notebook"
...
class TestSwaggerSpec:
    async def test_generation_of_swagger_spec(self, jp_fetch, jp_argv):
        ...
        assert (
            response.code == 200
        ), "Swagger spec endpoint did not return the correct status code"
        assert (
            result == expected_response
        ), "Swagger spec endpoint did not return the correct value"
        assert (
