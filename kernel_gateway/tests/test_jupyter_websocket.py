...
class TestDefaults:
    ...
    @pytest.mark.parametrize(
        "jp_argv", (["--JupyterWebsocketPersonality.list_kernels=True"],)
    )
    ...
    async def test_config_bad_api_value(
        self, jp_configurable_serverapp, jp_server_config
    ):
    ...
    async def test_auth_token(
        self, jp_server_config, jp_fetch, jp_web_app, jp_ws_fetch
    ):
        ...
        with pytest.raises(HTTPClientError) as e:
            await jp_fetch(
                "api", "kernelspecs", method="GET", headers={"Authorization": ""}
            )
        assert e.value.response.code == 401
        ...
        with pytest.raises(HTTPClientError) as e:
            await jp_fetch(
                "api", "kernels", kernel_id, method="GET", headers={"Authorization": ""}
            )
        assert e.value.response.code == 401
    ...
    async def test_max_kernels(self, jp_fetch, jp_web_app):
        ...
        kernel = json_decode(response.body)
        response = await jp_fetch(
            "api", "kernels", url_escape(kernel["id"]), method="DELETE"
        )
    ...
    @pytest.mark.parametrize(
        "jp_argv", (["--JupyterWebsocketPersonality.env_whitelist=TEST_VAR"],)
    )
...
@pytest.mark.parametrize(
    "jp_argv", (["--KernelGatewayApp.default_kernel_name=fake-kernel"],)
)
...
class TestForceKernel:
    ...
    async def test_force_kernel_name(self, jp_argv, jp_fetch):
        ...
        response = await jp_fetch(
            "api", "kernels", method="POST", body='{"name": "fake-kernel"}'
        )
        assert response.code == 201
        kernel = json_decode(response.body)
